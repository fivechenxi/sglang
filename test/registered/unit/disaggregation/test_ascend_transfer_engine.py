...
def test_batch_transfer_binds_executor_thread_to_engine_npu():
    ...
    fake_npu = SimpleNamespace(current_device=Mock(return_value=0), set_device=Mock())

    with (
        patch.object(ascend_transfer_engine.torch, "npu", fake_npu, create=True),
        patch.object(
    ...
    assert ret == 0
    fake_npu.set_device.assert_called_once_with(7)
...
def test_batch_transfer_does_not_rebind_correct_npu():
    engine = _build_engine_with_ports([12345])
    engine.npu_id = 7
    fake_npu = SimpleNamespace(current_device=Mock(return_value=7), set_device=Mock())
    ...
    fake_npu.set_device.assert_not_called()

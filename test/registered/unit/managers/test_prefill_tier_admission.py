...
def test_tier_limits_are_independent_and_release() -> None:
    ...
    assert admission.reserve_or_reconcile("cold", PrefillTierCost(cold_tokens=100))
    assert admission.reserve_or_reconcile("l2", PrefillTierCost(load_back_tokens=200))
    assert admission.reserve_or_reconcile("l3", PrefillTierCost(storage_tokens=300))
    assert not admission.reserve_or_reconcile(
        "extra-cold", PrefillTierCost(cold_tokens=1)
    )
    assert admission.exceeded_tiers("extra-cold", PrefillTierCost(cold_tokens=1)) == (
        "cold",
    )
    admission.release("cold")
    assert admission.reserve_or_reconcile("extra-cold", PrefillTierCost(cold_tokens=1))
...
def test_reconcile_is_atomic_when_cache_tier_changes() -> None:
    ...
    assert admission.reserve_or_reconcile("other", PrefillTierCost(cold_tokens=90))
    ...
    assert admission.used == PrefillTierCost(cold_tokens=90, load_back_tokens=80)
